(() => {
  ...
  const smoothNavigate = (target, hash) => {
    ...
    const duration = Math.min(1250, Math.max(680, Math.abs(distance) * 0.58));
    const started = performance.now();

    document.body.classList.add('section-travelling');
    const step = now => {
      const elapsed = Math.min(1, (now - started) / duration);
      window.scrollTo(0, start + distance * easeInOutQuint(elapsed));
      if (elapsed < 1) requestAnimationFrame(step);
      else {
        document.body.classList.remove('section-travelling');
  ...
  }, { rootMargin: '-28% 0px -58% 0px', threshold: [0.02, 0.2, 0.55] });
  linkedSections.forEach(section => activeNavObserver.observe(section));
